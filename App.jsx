...
function EVScanner({ match, onClose }) {
  ...
  const metCard = { flex: 1, background: "#f5f5f5", borderRadius: 8, padding: "10px 12px", textAlign: "center" };
  const metLabel = { fontSize: 11, color: "#888", marginBottom: 3 };
  const metVal = { fontSize: 20, fontWeight: 700 };
    ...
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "#fff", zIndex: 1000, overflow: "auto", maxWidth: 480, margin: "0 auto" }}>
      ...
      <div style={{ padding: "14px 14px 80px" }}>
        ...
        {tab === 'poisson' && (
          <div>
            ...
            <div style={{ background: "#f9f9f9", borderRadius: 10, border: "1px solid #eee", padding: 14, marginBottom: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#333", marginBottom: 12 }}>Match inputs</div>
              {[
                ['Home attack strength', atkH, setAtkH, 0.5, 2.5],
                ['Home defence strength', defH, setDefH, 0.5, 2.5],
                ['Away attack strength', atkA, setAtkA, 0.5, 2.5],
                ['Away defence strength', defA, setDefA, 0.5, 2.5],
                ['Minutes elapsed', minEl, setMinEl, 1, 90],
              ].map(([l, v, fn, mn, mx]) => (
                <div key={l} style={{ marginBottom: 10 }}>
                  ...
                  <input type="range" min={mn} max={mx} step={mx<=90?1:0.01} value={v} onChange={e => fn(+e.target.value)} style={{ width: "100%" }} />
                </div>
              ))}
            ...
            <div style={{ background: "#f9f9f9", borderRadius: 10, border: "1px solid #eee", padding: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#333", marginBottom: 12 }}>Distribution</div>
              {[
                ['P(0 more goals)', poi(0,xT)],
                ['P(exactly 1 more)', poi(1,xT)],
                ['P(exactly 2 more)', poi(2,xT)],
                ['P(3+ more goals)', 1-poiCum(2,xT)],
                ['P(over 0.5 — any goal)', probGoal],
                ['P(over 1.5 — 2+ goals)', 1-poiCum(1,xT)],
              ].map(([l,p]) => {
        ...
        {tab === 'ev' && (
          <div>
            <div style={{ background: "#f9f9f9", borderRadius: 10, border: "1px solid #eee", padding: 14, marginBottom: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#333", marginBottom: 12 }}>Inputs</div>
              <div style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#777", marginBottom: 3 }}>
                  <span>Your model probability</span><span style={{ fontWeight: 700, color: "#333" }}>{myProb}%</span>
                </div>
                <input type="range" min="1" max="99" step="1" value={myProb} onChange={e=>setMyProb(+e.target.value)} style={{ width: "100%" }} />
              </div>
              <div style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 12, color: "#777", marginBottom: 3 }}>Bookmaker decimal odds</div>
              ...
              <div>
                <div style={{ fontSize: 12, color: "#777", marginBottom: 3 }}>Minimum edge required</div>
                <select value={edgeThresh} onChange={e=>setEdgeThresh(+e.target.value)} style={{ width: "100%", padding: "8px 10px", border: "1px solid #e0e0e0", borderRadius: 6, fontSize: 13 }}>
            ...
            <div style={{ background: isBet ? "#e8f5e9" : "#ffebee", borderRadius: 10, border: `2px solid ${isBet?"#43a047":"#e53935"}`, padding: 14, marginBottom: 12 }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: isBet?"#1b5e20":"#b71c1c", marginBottom: 10 }}>
                {isBet ? "✅ BET — Positive EV" : "❌ SKIP — No edge"}
              </div>
              <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                {[['EV', evVal.toFixed(3)],['Edge', (edge>=0?'+':'')+edge.toFixed(1)+'%'],['Model', myProb+'%'],['Market', mktProb+'%'],['Kelly', kelly.toFixed(1)+'%']].map(([l,v]) => (
                  <div key={l} style={{ flex: 1, background: "#fff8", borderRadius: 6, padding: "6px 4px", textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 700, color: isBet?"#1b5e20":"#b71c1c" }}>{v}</div>
                  </div>
                ))}
              </div>
              <div style={{ fontSize: 12, color: isBet?"#2e7d32":"#c62828" }}>
                {edge >= 0 ? `Your model is ${Math.round(myProb-mktProb)}pp above market — theoretical edge exists.` : `Market implies higher probability. No value at these odds.`}
              </div>
            </div>
            <div style={{ background: "#f9f9f9", borderRadius: 10, border: "1px solid #eee", padding: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#333", marginBottom: 10 }}>Example live opportunities</div>
              {[
                {m:'Arsenal vs Man City',mkt:'Over 0.5',mdl:74,odds:1.62,min:82},
                {m:'Barcelona vs Real Madrid',mkt:'Next goal',mdl:58,odds:2.10,min:71},
                {m:'PSG vs Lyon',mkt:'Over 0.5',mdl:61,odds:1.75,min:68},
                {m:'Dortmund vs Bayern',mkt:'Over 1.5',mdl:42,odds:2.80,min:55},
              ].map((g,i) => {
                const ev2 = g.mdl/100*g.odds;
                const e2 = (ev2-1)*100;
                const bet2 = ev2 >= 1.05;
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: i<3?"1px solid #eee":"none" }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#333" }}>{g.m} <span style={{ fontSize: 11, color: "#aaa" }}>{g.min}'</span></div>
                      <div style={{ fontSize: 12, color: "#888" }}>{g.mkt} @ {g.odds}</div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: e2>=5?"#2e7d32":e2>0?"#e65100":"#c62828" }}>{e2>=0?'+':''}{e2.toFixed(1)}%</div>
                      <div style={{ fontSize: 11, background: bet2?"#e8f5e9":"#ffebee", color: bet2?"#2e7d32":"#c62828", borderRadius: 10, padding: "2px 8px", fontWeight: 700 }}>{bet2?'Bet':'Skip'}</div>
                    </div>
                  </div>
                );
              })}
        ...
        {tab === 'strength' && (
          <div>
            <div style={{ background: "#e3f2fd", borderRadius: 8, padding: 12, marginBottom: 12, fontSize: 13, color: "#1565c0" }}>
              <strong>Formula:</strong> Attack strength = team avg goals ÷ league avg. Defence strength = avg conceded ÷ league avg. Lower defence = better. League avg: 1.36 home / 1.06 away.
            </div>
            {teams.map((t,i) => {
              const tc = {Elite:"#2e7d32",Top:"#1565c0","Mid-top":"#e65100",Mid:"#888","Low-mid":"#c62828"}[t.t]||"#888";
              return (
                <div key={i} style={{ background: "#f9f9f9", borderRadius: 10, border: "1px solid #eee", padding: 12, marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: "#111" }}>{t.n}</span>
        ...
        {tab === 'backtest' && (
          <div>
            ...
            <div style={{ background: "#f9f9f9", borderRadius: 10, border: "1px solid #eee", padding: 14, marginBottom: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#333", marginBottom: 10 }}>Log new alert</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <input type="text" value={btMatch} onChange={e=>setBtMatch(e.target.value)} placeholder="Match name (e.g. Arsenal vs Chelsea)" style={{ padding: "8px 10px", border: "1px solid #e0e0e0", borderRadius: 6, fontSize: 13 }} />
                <div style={{ display: "flex", gap: 8 }}>
                  {[['Minute',btMin,setBtMin,1,93],['Heat',btHeat,setBtHeat,0,100],['Prob %',btProb,setBtProb,1,99]].map(([l,v,fn,mn,mx])=>(
                    <div key={l} style={{ flex: 1 }}>
                      <div style={{ fontSize: 11, color: "#888", marginBottom: 3 }}>{l}</div>
                      <input type="number" min={mn} max={mx} value={v} onChange={e=>fn(+e.target.value)} style={{ width: "100%", padding: "7px 8px", border: "1px solid #e0e0e0", borderRadius: 6, fontSize: 13 }} />
                    </div>
                  ))}
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <div style={{ flex: 2 }}><div style={{ fontSize: 11, color: "#888", marginBottom: 3 }}>Outcome</div>
                    <select value={btOut} onChange={e=>setBtOut(e.target.value)} style={{ width: "100%", padding: "7px 8px", border: "1px solid #e0e0e0", borderRadius: 6, fontSize: 13 }}>
                      ...
                      <option value="yes">Goal scored ✅</option>
            ...
            <div style={{ background: "#f9f9f9", borderRadius: 10, border: "1px solid #eee", padding: 14, marginBottom: 12 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#333", marginBottom: 8 }}>Window analysis</div>
              {windows.map(w => {
                ...
                const rating = r===null?'—':r>=70?'⭐ Strong':r>=50?'✓ Good':'⚠ Weak';
                const rc = r===null?"#888":r>=70?"#2e7d32":r>=50?"#e65100":"#c62828";
                return (
                  <div key={w.l} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px solid #eee" }}>
                    ...
                    <span style={{ fontSize: 12, color: rc }}>{rating}</span>
                  </div>
                );
              })}
            </div>
            {alerts.length === 0 ? (
              <div style={{ textAlign: "center", color: "#bbb", padding: "2rem", fontSize: 13 }}>No alerts logged yet. Start logging to track accuracy.</div>
            ) : (
              <div style={{ background: "#f9f9f9", borderRadius: 10, border: "1px solid #eee", padding: 14 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#333", marginBottom: 8 }}>Recent alerts</div>
                  ...
                  return (
                    <div key={a.id} style={{ padding: "8px 0", borderBottom: "1px solid #eee" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: 13, fontWeight: 600, color: "#333" }}>{a.m}</span>
                        <span style={{ fontSize: 11, color: a.out==='yes'?"#2e7d32":a.out==='no'?"#c62828":"#888", fontWeight: 700 }}>{a.out==='yes'?'✅ Goal':a.out==='no'?'❌ Miss':'⏳ Pending'}</span>
                      </div>
                      <div style={{ fontSize: 12, color: "#888" }}>{a.min}′ · Heat {a.heat} · Prob {a.prob}% · Odds {a.odds} · EV {ev2>=0?'+':''}{ev2.toFixed(1)}%</div>
                    </div>
                  );
                })}
              </div>
