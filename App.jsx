  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "#000000cc", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#111", border: "1px solid #ffffff14", borderRadius: 14, padding: 24, width: "100%", maxWidth: 320 }}>
        ...
        <div style={{ fontSize: 11, color: "#555", fontFamily: "monospace", marginBottom: 20 }}>Notify when Heat Score ≥</div>
...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!json.matches || json.matches.length === 0) {
        ...
        setMatches(prev => {
          return json.matches.map(m => {
            // Use heat_score from backend — don't recalculate
      ...
      setIsDemo(true);
      setMatches(DEMO);
      setError(e.message.includes("RAPIDAPI_KEY") ? "no_key" : "api_error");
    } finally {
      setLoading(false);
        ...
        <div style={{ maxWidth: 820, margin: "0 auto" }}>
          <div className="hrow" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10, gap: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 9, borderRadius: 4, padding: "2px 7px", fontFamily: "monospace", border: `1px solid ${isDemo ? "#ffd60a33" : "#30d15833"}`, background: isDemo ? "#ffd60a18" : "#30d15818", color: isDemo ? "#ffd60a" : "#30d158" }}>
                {isDemo ? "DEMO" : "● LIVE"}
              </span>
            </div>
          ...
          {/* Error banners */}
          {error === "no_key" && (
            <div style={{ marginBottom: 8, padding: "8px 12px", background: "#ff3b3012", border: "1px solid #ff3b3033", borderRadius: 8, fontSize: 11, color: "#ff6b6b", fontFamily: "monospace" }}>
              ⚠️ <strong>RAPIDAPI_KEY</strong> not set — go to Vercel → Settings → Environment Variables → add <strong>RAPIDAPI_KEY</strong> → Redeploy
            </div>
          )}
          {error === "no_matches" && (
            <div style={{ marginBottom: 8, padding: "8px 12px", background: "#ffd60a0a", border: "1px solid #ffd60a22", borderRadius: 8, fontSize: 11, color: "#ffd60a", fontFamily: "monospace" }}>
              ⚽ No live matches right now — showing demo. Reloads automatically when games kick off.
            </div>
          )}
          {error === "api_error" && (
            <div style={{ marginBottom: 8, padding: "8px 12px", background: "#ff3b3012", border: "1px solid #ff3b3033", borderRadius: 8, fontSize: 11, color: "#ff6b6b", fontFamily: "monospace" }}>
          ...
          <div style={{ fontSize: 10, color: "#2a2a2a", fontFamily: "monospace", marginTop: 6 }}>{isDemo ? "● DEMO" : "● LIVE"} · {matches.length} matches · refresh #{tick}</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 820, margin: "0 auto", padding: "14px 16px 40px" }}>
        ...
        {isDemo && (
          <div style={{ marginTop: 24, padding: 14, borderRadius: 10, background: "#0d0d0d", border: "1px solid #ffffff06" }}>
            <div style={{ fontSize: 10, color: "#2a2a2a", fontFamily: "monospace", marginBottom: 8, letterSpacing: "0.08em" }}>HOW TO GO LIVE</div>
            <div style={{ fontSize: 10, color: "#333", fontFamily: "monospace", lineHeight: 2 }}>
              ...
              3. Add: <span style={{ color: "#30d158" }}>RAPIDAPI_KEY</span> = your_key_here<br />
              4. Redeploy → banner switches DEMO → ● LIVE
            </div>
          </div>
        )}
      </div>
